...
def experiment_once(packets):
    ...
    # Add random noise
    noise = np.random.laplace(10, 3, size=(len(watermarked_packets), ))
    ...
    for i in range(n):
        base_start_time = i * 2 * T
        mark_start_time = base_start_time + T
        base_interval = subinterval_slice(base_start_time, watermarked_packets)
        if len(base_interval) == 0:
            continue
        s = get_s(base_start_time, base_interval)
        s_prime = get_s_prime(base_start_time, base_interval)

        if detect(mark_start_time, s, watermarked_packets) or detect(
                mark_start_time, s_prime, watermarked_packets):
            d += 1
